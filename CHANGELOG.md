...
- Status bar block now has two click targets like git's branch + sync: clicking the counter opens the action menu, clicking the ⟳ refreshes — one compound block, no separate widgets (a PTAL section appears in the Source Control panel as a side effect)
